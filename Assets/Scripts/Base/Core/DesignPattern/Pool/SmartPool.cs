...
//--------------------------------------------------------------------------------------
    ...
    public class Pool
    {
        ...
        /// </summary>
        /// <returns></returns>
        public void EnsureContainerUnder(Transform parent)
        {
            if (_prefabContrainer.transform.parent == parent)
                return;

            _prefabContrainer.transform.SetParent(parent, false);

            if (parent is RectTransform)
            {
                var containerRect = _prefabContrainer.GetComponent<RectTransform>();
                if (containerRect == null)
                    containerRect = _prefabContrainer.AddComponent<RectTransform>();
    ...
    public class SmartPool : SingletonMono<SmartPool>
    {
        ...
        public GameObject SpawnUI(GameObject prefabs, RectTransform parent, Vector2? anchoredPosition = null)
        {
            if (!prefabs || !parent)
                return null;

            Init(prefabs);
            var pool = instance._pools[prefabs];
            pool.EnsureContainerUnder(parent);

            var obj = pool.Spawn(Vector3.zero, Quaternion.identity, Vector3.one);
            if (obj == null)
                return null;

            var rect = obj.GetComponent<RectTransform>();
            if (rect != null)
                ApplyRectTransformFromPrefab(rect, prefabs, parent, anchoredPosition);
            else
        ...
        private static void ApplyRectTransformFromPrefab(
            ...
            Vector2? anchoredPosition)
        {
            ...
            if (prefabRect != null)
            {
                instance.anchorMin = prefabRect.anchorMin;
                instance.anchorMax = prefabRect.anchorMax;
                instance.pivot = prefabRect.pivot;
                instance.sizeDelta = prefabRect.sizeDelta;
                instance.localRotation = prefabRect.localRotation;
                instance.localScale = prefabRect.localScale;
                instance.anchoredPosition = anchoredPosition ?? prefabRect.anchoredPosition;
            }
            else
            {
                instance.localScale = Vector3.one;
                instance.anchoredPosition = anchoredPosition ?? Vector2.zero;
            }
        }
